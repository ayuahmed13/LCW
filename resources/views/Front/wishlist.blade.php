                    ...
                    <div class="wrap-sidebar-account">
                        ...
                        <div class="account-avatar">
                            ...
                            <ul class="my-account-nav">
                                ...
                                <li>
                                    <a href="{{ url('wishlist') }}" class="my-account-nav-item active">
                         ...
                         <div class="card-product wow fadeInUp" data-wow-delay="0s" data-availability="Out of stock" data-brand="adidas">
                            <div class="card-product-wrapper">
                                <a href="{{ url('product-detail') }}/{{!empty($value->slug_url)?($value->slug_url):''}}" class="product-img">
                                    <img class="lazyload img-product" data-src="{{ !empty($value->product_main_image) && Storage::exists($value->product_main_image) ? url('/').Storage::url($value->product_main_image) : URL::asset('front/images/default-img.jpg') }}" src="{{ !empty($value->product_main_image) && Storage::exists($value->product_main_image) ? url('/').Storage::url($value->product_main_image) : URL::asset('front/images/default-img.jpg') }}" alt="image-product">
                                    <img class="lazyload img-hover" data-src="{{ !empty($value->product_main_image) && Storage::exists($value->product_main_image) ? url('/').Storage::url($value->product_main_image) : URL::asset('front/images/default-img.jpg') }}" src="{{ !empty($value->product_main_image) && Storage::exists($value->product_main_image) ? url('/').Storage::url($value->product_main_image) : URL::asset('front/images/default-img.jpg') }}" alt="image-product">
                                </a>
                                <div class="list-product-btn">
                                    <a href="javascript:void(0);" class="box-icon wishlist btn-icon-action remove-from-wishlist" data-id="{{!empty($value->id)?Crypt::encrypt($value->id):''}}" style="background-color: black;color: white;">
                                        <span class="icon icon-heart"></span>
                                        <span class="tooltip">Remove</span>
                                    </a>
                                </div>
                                <div class="list-btn-main">
                                @if((!empty($value->status) && $value->status!='active') || !empty($value->is_available) && $value->is_available!='available')
                                <label class="btn btn-dark text-white p-1" ><b>(Not Available)</b></label>
                                @else
                                    @if((!empty($cart_product_ids) && in_array($value->pid,$cart_product_ids)) || (empty($tus) && !empty($cart) && $inCart = array_key_exists($value->pid, $cart)))
                                    <a href="{{url('/')}}/shopping-cart" class="btn-main-product btn btn-primary w-100 go-to-cart" 
                                                    data-product-id="{{ !empty($value->pid)?$value->pid:''}}"
                            ...
                            <div class="card-product-info">
                                ...
                                <span class="price current-price">${{!empty($value->offer_price)?$value->offer_price:''}}</span>
                            </div>
                        </div>
                        @endforeach
                        </div>
                        @else
                        <div class=" text-center mt-4 p-5 bg-dark text-white rounded" style="    width: 100%;height: fit-content;">
